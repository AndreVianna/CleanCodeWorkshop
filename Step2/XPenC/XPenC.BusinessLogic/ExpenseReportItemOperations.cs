...
namespace XPenC.BusinessLogic
{
    public class ExpenseReportItemOperations : IExpenseReportItemOperations
    {
        ...
        private void ValidateAddOperation(ExpenseReportItem input)
        {
            ...
            ValidateItemDate(validator, input);
            ValidateItemValue(validator, input);
            validator.Validate();
        }

        private static void ValidateItemDate(OperationValidator validator, ExpenseReportItem input)
        {
        ...
        private static void ValidateItemValue(OperationValidator validator, ExpenseReportItem input)
        {
            if (input.Value <= 0)
            {
        ...
        internal static void ProcessItemRules(ExpenseReportItem item)
        {
